...
def manifest(request, uuid):
    ...
    """
    try:
        webapp = (Webapp.objects.all().only_translations()
                  .get(disabled_by_user=False, is_packaged=True, guid=uuid))
    except Webapp.DoesNotExist:
        raise http.Http404

    is_available = webapp.status in [mkt.STATUS_PUBLIC, mkt.STATUS_UNLISTED,
                                     mkt.STATUS_BLOCKED]
    if is_available:
        ...
        is_owner = webapp.authors.filter(pk=request.user.pk).exists()
        is_available_to_owner = webapp.status == mkt.STATUS_APPROVED

    if (is_available or (is_available_to_owner and is_owner)):

        manifest_content, manifest_etag = webapp.get_cached_manifest()

        @etag(lambda r, a: manifest_etag)
        def _inner_view(request, webapp):
            response = http.HttpResponse(manifest_content,
                                         content_type=MANIFEST_CONTENT_TYPE)
            return response

        return _inner_view(request, webapp)
