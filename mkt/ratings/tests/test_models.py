...
class TestSpamTest(mkt.site.tests.TestCase):
    ...
    def setUp(self):
        ...
        Review.objects.create(webapp=self.app, user=self.user,
                              title="review 1", rating=3)
        Review.objects.create(webapp=self.app, user=self.user,
                              title="review 2", rating=2)
    ...
    @patch('mkt.ratings.tasks.webapp_review_aggregates.delay')
    def test_refresh_triggers_review_aggregates(self,
                                                webapp_review_aggregates):
        webapp_review_aggregates.reset_mock()
        review = Review.objects.latest('pk')
        review.refresh()
        assert webapp_review_aggregates.called
    ...
    def test_review_aggregates_triggers_reindex(self, index_webapps):
        ...
        webapp_review_aggregates(self.app.pk)
