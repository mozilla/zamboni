...
def purchase_list(request, user):
    ...
    # they have installed.
    def get_ids(qs):
        return list(qs.order_by('-id').values_list('webapp_id', flat=True))
    ...
    installed_apps_ids = get_ids(user.installed_set.filter(install_type__in=[
        ...
        webapp__in=contributed_apps_ids))

    webapp_ids = contributed_apps_ids + installed_apps_ids
    qs = Webapp.objects.filter(id__in=webapp_ids)
    products = paginate(request, manual_order(qs, webapp_ids),
                        count=qs.count())
    return products
